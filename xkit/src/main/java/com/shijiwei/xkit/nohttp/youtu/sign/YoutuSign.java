...
public class YoutuSign {
	...
	public static int appSign(String appId, String secret_id, String secret_key,
		...
		return appSignBase(appId, secret_id, secret_key, expired, "664588927", null, mySign);
	}

    public static int sign(StringBuffer sign){
        return appSign(YouTuConfig.YT_APP_ID, YouTuConfig.YT_SECRET_ID,YouTuConfig.YT_SECRET_KEY, System.currentTimeMillis() / 1000 + EXPIRED_SECONDS,"" ,sign);
    }
	...
	private static int appSignBase(String appId, String secret_id,
		...
		if (empty(secret_id) || empty(secret_key)) return -1;
