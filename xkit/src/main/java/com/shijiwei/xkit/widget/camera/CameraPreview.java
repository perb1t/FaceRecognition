package com.shijiwei.xkit.widget.camera;
 ...
 */
public class CameraPreview extends SurfaceView implements SurfaceHolder.Callback, Camera.PictureCallback, AspectRatioViewInterface, Camera.AutoFocusCallback, Camera.PreviewCallback {
...
//            mHardwareCamera.getParameters().setPreviewSize(720, 1);
            degree = setCameraDisplayOrientation((Activity) getContext());
            ...
            //设置是否预览参数为真
            isPreviewing = true;
            mHardwareCamera.setPreviewCallback(this);
        } catch (IOException e) {
...
//        setCameraDisplayOrientation((Activity) getContext());
        if (mCameraPreviewCallBack != null) mCameraPreviewCallBack.onPreviewFrame(data, camera);
    ...
    private void zoomPicture(final byte[] data, final int size) {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                if (zoom < 1) {
                    mPicture = rotateBitmap2ZeroDegree(bitmap, degree);
                    mCompressHandler.sendEmptyMessage(0);
                    return;
                }
                ...
                // 照片旋转到正常角度
                mPicture = rotateBitmap2ZeroDegree(bitmap2, degree);
                mCompressHandler.sendEmptyMessage(0);
            }
        }).start();
    }
